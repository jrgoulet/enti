            ...
            for default in defaults:
                ...
                try:
                    ...
                    if def_existing is None:
                        log.info('Initializing default {}: {}'.format(def_type.__name__, default.name))
                        session.add(default)

                    else:
                        log.info('Entry exists for {}: {}'.format(def_type.__name__, default.name))
        ...
        for attr_id, attr_template in attr_templates.items():
            ...
            try:
                ...
                if attribute_existing is None:
                    log.info('Initializing attribute: {}'.format(attribute.name))
                ...
                for field_id in attr_template['fields']:
                    ...
                    if attr_field is not None:
                        ...
                        if linked_attr_field_existing is None:
                            log.info('Creating linked attribute field {} for attribute {}'.format(attr_field.name,
                                                                                                  attribute.name))
